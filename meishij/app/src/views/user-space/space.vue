...
export default {
  ...
  watch: {
    $route: {
      async handler() {
        let { userId } = this.$route.query; //访问自己空间 或 userId为自身id
  ...
  methods: {
    getInfo() {
      /*
       * 在快速切换tab的时候，由于Ajax延迟，tab对应的Ajax返回才是填充的数据
       * 1.用标识确认
       * 2.用数据暂存this.queen
       */
      (async activeName => {
    ...
    async toggleHandler() {
      const res = await toggleFollowing({ followUserId: this.userInfo.userId });
      this.userInfo = res.data;
    },
    tabClickHandler() {
      this.list = []; //切换前清空list，不同tab填充不同list
      this.$router.push({
